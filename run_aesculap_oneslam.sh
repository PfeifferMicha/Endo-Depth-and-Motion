...
do
    ...
    if [ -d "$patient_orig_path" ]; then
        ...
        tracking_result=$(ls -t $patient_output_path/OneSLAM/* | head -1)
        # For some "§$%&$/&%"§&$§/ reason, OneSlam places colons into its files which confuses ls and head. Remove the 
        # trailing ':':
        tracking_result=$(echo "$tracking_result" | sed 's:\:*$::')
        echo Tracking result: $tracking_result
        tracking_result=$tracking_result/poses.pickle
        echo Tracking result: $tracking_result
        ...
        conda init
        conda activate endo_depth_and_motion
        python3 apps/volumetric_fusion/__main__.py -i $tracking_result -o data/Aesculap/$patient/ --use_poses_from_oneslam
    fi
